...
public class WebDriverManager  {
    ...
    public void initSauceDriver(String browser, String version, String os) throws MalformedURLException {
        ...
        if(runOnSauceLabs) {
	        ...
	        WebDriverFactory.setWebDriver(new RemoteWebDriver(
	                new URL("http://" + authentication.getUsername() + ":" + authentication.getAccessKey() + "@ondemand.saucelabs.com:80/wd/hub"),
	                capabilities));
	        WebDriverFactory.setSessionID(((RemoteWebDriver) WebDriverFactory.getDriver()).getSessionId().toString());
        }
    ...
    public void initDriver(HashMap<String, String> configMap) throws Exception {
        ...
        if(runOnSauceLabs) {
        	...
        	WebDriverFactory.setWebDriver(new RemoteWebDriver(new URL(wdHuburl), capabilities));
        	WebDriverFactory.setSessionID( ((RemoteWebDriver) WebDriverFactory.getDriver()).getSessionId().toString());
        	...
        	switch(browserType) {
	        	case FF:
	        		WebDriverFactory.setWebDriver(new FirefoxDriver(capabilities));
					break;
					
				case IE:
					capabilities.setCapability(InternetExplorerDriver.INTRODUCE_FLAKINESS_BY_IGNORING_SECURITY_DOMAINS,true);
					WebDriverFactory.setWebDriver(new InternetExplorerDriver(capabilities));
					break;
				...
				case CHROME:
					if(configMap.containsKey(CHROME_DRIVER_PATH)) {
						...
						WebDriverFactory.setWebDriver(new ChromeDriver(capabilities));
					} else {
